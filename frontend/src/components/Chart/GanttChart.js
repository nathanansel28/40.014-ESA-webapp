...
const GanttChart = () => {
  ...
  useEffect(() => {
    ...
    if (storedChartData) {
      ...
      const fetchData = async () => {
        try {
          const response = await axios.get('http://127.0.0.1:8000/static/files/best_schedule2.csv', {
            responseType: 'blob',
          });
  ...
  return (
    <div className="gantt-chart-container">
      ...
      <div ref={chartRef} style={{ width: '100%', height: '500px' }}></div> {/* Ensure the div has dimensions */}
    </div>
  );
};
