...
function Manage() {
  ...
  const [dataframeBom, setDataframeBom] = useState(null);
  const [dataframeWorkcentre, setDataframeWorkcentre] = useState(null);
        ...
        Papa.parse(text, {
          ...
          complete: async (result) => {
            if (type === 'bom') {
              ...
              const response = await axios.post('http://127.0.0.1:5000/convert_to_dataframe_bom', result.data);
              setDataframeBom(response.data.dataframe);
            } else {
              setCsvData1(result.data);
              sessionStorage.setItem('csvData1', JSON.stringify(result.data));
              const response = await axios.post('http://127.0.0.1:5000/convert_to_dataframe_workcentre', result.data);
